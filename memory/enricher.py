...
class SweepEngine:
    ...
    def sweep_patterns(self, batch_size: int = 500) -> list[dict]:
        ...
        # ── Recurring technical patterns (Apple Silicon ML domain) ──
        provision_entities = defaultdict(set)
        provision_pattern = re.compile(
            r'\b(dispatch\s*floor|fusion\s*depth|residual\s*accumulation'
            r'|prompt\s*cache|spec\s*decode|n[- ]gram\s*draft'
            r'|kv\s*cache|prefix\s*cache|coreml\s*pipeline)\b',
            re.IGNORECASE
        )
        for entity, data in entity_data.items():
